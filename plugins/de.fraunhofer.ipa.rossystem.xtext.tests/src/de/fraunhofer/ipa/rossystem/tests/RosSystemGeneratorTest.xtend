...
class RosSystemGeneratorTest {
	...
	@Test
	def void testGeneratedComponentStackCode() {
		...
		val fileContent = new String(Files.readAllBytes(Paths.get(RESOURCES_BASE_DIR, String.format("%s.rossystem",system_name ))))
		val model = parseHelper.parse(fileContent, resourceSet)
		val fsa = new InMemoryFileSystemAccess
